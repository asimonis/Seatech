...
for(d in 1:length(drifts)){
  ...
  LogUO<-filter(Log,Species.Code=="UO")
   for(m in 1:nrow(Log)){
  ...
  TimesDF$Gg<-'Absent'
  #Loop through times to see if UO events span "recording on" periods
  LogGg<-filter(Log,Species.Code=="Gg")
   for(m in 1:nrow(Log)){
...
PlotPresBars<-function(figtitle){
  ...
  plotPresBar(UODF,start=min(GPSDF$UTC),end=max(GPSDF$UTC),bin="day",by="Species",title=paste0(figtitle,': Dolphins'),fill=Species)+
...
PlotPresMaps<-function(figtitle){
  ...
  autoplot.bathy(bat,geom=c('r'),coast=TRUE)+
    ...
    new_scale_color()+
    geom_point(data=UODF,aes(x=Longitude,y=Latitude,color=Species))+scale_color_manual(values=c(UOColor), labels=c("Unidentified Dolphins"),name="")+
...
#Function PlotPresMaps will map Gg events from Drift 10

  GgDF<-(TimesAllGg) %>%
    filter(Drift=='Drift-10')
  ...
  autoplot.bathy(bat,geom=c('r'),coast=TRUE)+
    ...
    geom_point(data=GPSDF,aes(Longitude,Latitude),size=0.1)+
    new_scale_color()+
    geom_point(data=GgDF,aes(x=Longitude,y=Latitude,color=GgColor))+scale_color_manual(values=c(GgColor), labels=c("Risso\'s Dolphins"),name="")+
    xlab('Longitude')+ylab('Latitude')+ggtitle('Drift-10')
...
AllSpecies<-rbind(UODF_gps,GgDF)

#define boundaries of the map
lon1=-132;lon2=-114;lat1=30;lat2=40
...
autoplot.bathy(bat,geom=c('r'),coast=TRUE)+
  ...
  geom_point(data=GPSAll,aes(Longitude,Latitude),size=0.1)+
  new_scale_color()+
  geom_point(data=AllSpecies,aes(x=Longitude,y=Latitude,color=Species))+
  scale_color_manual(values=c(UOColor,GgColor), labels=c("Unidentified Dolphins",'Risso\'s Dolphins'),name="")+
  xlab('Longitude')+ylab('Latitude')+ggtitle('All Drifts')
...
autoplot.bathy(bat,geom=c('r'),coast=TRUE)+
  ...
  geom_point(data=GPSAll,aes(Longitude,Latitude),size=0.1)+
  new_scale_color()+
  geom_point(data=GgDF,aes(x=Longitude,y=Latitude))+
  scale_color_manual(values=c(GgColor), labels=c("Risso\'s Dolphins"),name="")+
  xlab('Longitude')+ylab('Latitude')+ggtitle('All Drifts')
...
daybins = seq(as.POSIXct(min(TimesAllUO$UTC)),as.POSIXct(max(TimesAllUO$UTC)),'days')
...
countdf = data.frame(daybins)
if (iD > 1){
  countdf$counts = countdata$cnt
  countdf$id = rep(figtitle[iD],length(daybins))

  whalecounts = rbind(whalecounts,countdf)
}else{
  whalecounts$counts = countdata$cnt
whalecounts$id = rep(figtitle[iD],length(daybins))
}
}

#change zeros to nans so they don't plot
whalecounts$counts[whalecounts$counts==0]=NA

#plotting
ggplot(data=whalecounts,aes(x=daybins,y=counts))+ 
geom_point()+
