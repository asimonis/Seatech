...
suppressMessages(pacman::p_load(knitr,here,lubridate,dplyr,magrittr,xlsx,RSQLite,viridis,marmap,ggplot2,suncalc,ggnewscale,PAMmisc,stringr,gridExtra))
...
PlotPresDrifts<-function(figtitle){
  ...
  DielMn = plotPresGrid(MnDF,start=min(GPSDF$UTC),end=max(GPSDF$UTC),type="presence",
               bin="minute",gps=GPSDF,title=paste0(figtitle,': Humpback whales'),fill=MnColor)

  DielShip = plotPresGrid(AnthroDF,start=min(GPSDF$UTC),end=max(GPSDF$UTC),type="presence",
               bin="minute",gps=GPSDF,title=paste0(figtitle,': Ships'),fill=ShipColor)
...
PlotPresBars<-function(figtitle){
  ...
  plotPresBar(SpDF,start=min(GPSDF$UTC),end=max(GPSDF$UTC),bin="day",by="Species",title=paste0(figtitle,': Humpback whales and ships'),fill=Species)+
