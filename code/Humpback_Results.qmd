...
for(d in 1:length(drifts)){
  ...
  #truncate log down to just detections from this drift
  Loguse = Log[Log$Drift==drifts[d],]
  ...
  for(m in 1:nrow(Loguse)){
    IndStart<-max(which(TimesDF$Start<=Loguse$Start.time[m]))
    IndStop<-min(which(TimesDF$Stop>=Loguse$End.time[m]))
     # IndStop<-min(TimesDF$Stop>=Loguse$End.time[m])
    if(IndStart>0){
      if(is.na(IndStop) | IndStop==0 | is.infinite(IndStop)){IndStop=IndStart}
      TimesDF$Presence[IndStart:IndStop]<-'Present'
      TimesDF$Species[IndStart:IndStop]<-rep(Loguse$Species.Code[m],length(IndStart:IndStop))
    }
  }              
...
for(d in 1:length(drifts)){
  DriftID<-paste0('Drift-',drifts[d])
 #remove events 
 removeEventInd<-which(TimesAll$Drift==DriftID & TimesAll$UTC>EndDates[d])
 TimesAll<-TimesAll[-removeEventInd,]
 remove(removeEventInd)
 
 #remove GPS data
 removeGPSInd<-which(GPSAll$Drift==DriftID & GPSAll$UTC>EndDates[d])
 GPSAll<-GPSAll[-removeGPSInd,]
 remove(removeGPSInd)
}  
}
```
